        ...
        try(BufferedReader reader = new BufferedReader(new FileReader(albumFile)))
        {
            ...
            while(line != null)
            {
                if(line.trim().length() > 0) // Ignore blank lines
                {
                    ...
                    ImageRecord image = new ConcreteImage(imageFilename, imageCaption);

                    if(parts.length > 2)
                    {
                        for(int i = 2; i< parts.length; i++)
                        {
                            String[] label = parts[i].split("=", 2);
                            switch(label[0])
                            {
                                case "date":
                                    date = label[1];
                                    image = new DecDate(image, date);
                                break;
                                case "gps":
                                    gps = label[1];
                                    image = new DecGPS(image, gps);
                                break;
                                case "rating":
                                    rating = label[1];
                                    image = new DecRating(image, rating);
                                break;
                            }
                        }
                    }
                    
                    // #1 FIXME: Insert your code here to add a new image to the album.
                    album.addImage(image);
                    /**
                     * An instance of an ImageRecord is created
                     * Then it is added to a hash map storing a collection of ImageRecords
                     * using the image name as the key
                     */
                }
